...
impl DeliveryProvider for DhlProvider {
    ...
    fn create_shipment<'a>(
        ...
        Box::pin(async move {
            info!("DHL: Delegating shipment {} to Playwright microservice", req.order_number);
            ...
            let res = self.client
                .post("http://127.0.0.1:3211/api/dhl/create")
                .json(&payload)
                .send()
                .await?;
            ...
            if !res.status().is_success() {
                ...
                warn!("DHL Scraper failed: {}", err_text);
                return Err(anyhow::anyhow!("DHL Scraper Service Error: {}", err_text));
            }
            ...
            let tracking_number = data["tracking_number"]
                .as_str()
                .unwrap_or(&format!("DHL-UNKNOWN-{}", req.order_number))
                .to_string();
